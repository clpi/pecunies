...
export async function onRequestGet({ request, env }) {
  ...
  if (slug && slug !== "command") {
    const result = await d1.prepare(
      "SELECT * FROM catalog_entities WHERE type = 'command' AND slug = ?"
    ).bind(slug).first();
    ...
    const usedByResult = await d1.prepare(`
      ...
      WHERE cr.to_type = 'command' AND cr.to_slug = ?
  ...
  const result = await d1.prepare(
    "SELECT * FROM catalog_entities WHERE type = 'command' ORDER BY slug"
  ).all();
...
export async function onRequestPost({ request, env }) {
  ...
  try {
    ...
    if (action === "delete") {
      ...
      await d1.prepare("DELETE FROM catalog_entities WHERE type = 'command' AND slug = ?")
        .bind(slug).run();
      await d1.prepare("DELETE FROM catalog_relations WHERE from_type = 'command' AND from_slug = ?")
        .bind(slug).run();
      await d1.prepare("DELETE FROM catalog_relations WHERE to_type = 'command' AND to_slug = ?")
        .bind(slug).run();
