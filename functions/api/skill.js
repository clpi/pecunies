...
export async function onRequestGet({ request, env }) {
  ...
  if (slug && slug !== "skill") {
    ...
    const result = await d1.prepare(
      "SELECT * FROM catalog_entities WHERE type = 'skill' AND slug = ?"
    ).bind(slug).first();
    ...
    const usedByResult = await d1.prepare(`
      ...
      WHERE cr.to_type = 'skill' AND cr.to_slug = ?
  ...
  // List all skills
  const result = await d1.prepare(
    "SELECT * FROM catalog_entities WHERE type = 'skill' ORDER BY slug"
  ).all();
...
export async function onRequestDelete({ request, env }) {
  ...
  await d1.prepare("DELETE FROM catalog_entities WHERE type = 'skill' AND slug = ?")
    .bind(slug).run();
  await d1.prepare("DELETE FROM catalog_relations WHERE from_type = 'skill' AND from_slug = ?")
    .bind(slug).run();
  await d1.prepare("DELETE FROM catalog_relations WHERE to_type = 'skill' AND to_slug = ?")
    .bind(slug).run();
...
export async function onRequestPost() {
  return Response.json({ error: "Use PUT for updates, DELETE for deletion" }, { status: 405, headers: jsonHeaders });
}