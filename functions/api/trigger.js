...
export async function onRequestGet({ request, env }) {
  ...
  if (slug && slug !== "trigger") {
    const result = await d1.prepare(
      "SELECT * FROM catalog_entities WHERE type = 'trigger' AND slug = ?"
    ).bind(slug).first();
    ...
    const usedByResult = await d1.prepare(`
      ...
      WHERE cr.to_type = 'trigger' AND cr.to_slug = ?
  ...
  const result = await d1.prepare(
    "SELECT * FROM catalog_entities WHERE type = 'trigger' ORDER BY slug"
  ).all();
...
export async function onRequestPost({ request, env }) {
  ...
  try {
    ...
    if (action === "delete") {
      ...
      await d1.prepare("DELETE FROM catalog_entities WHERE type = 'trigger' AND slug = ?")
        .bind(slug).run();
      await d1.prepare("DELETE FROM catalog_relations WHERE from_type = 'trigger' AND from_slug = ?")
        .bind(slug).run();
      await d1.prepare("DELETE FROM catalog_relations WHERE to_type = 'trigger' AND to_slug = ?")
        .bind(slug).run();
