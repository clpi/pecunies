...
export async function onRequestGet({ request, env }) {
  ...
  if (slug && slug !== "view") {
    const result = await d1.prepare(
      "SELECT * FROM catalog_entities WHERE type = 'view' AND slug = ?"
    ).bind(slug).first();
    ...
    const usedByResult = await d1.prepare(`
      ...
      WHERE cr.to_type = 'view' AND cr.to_slug = ?
  ...
  const result = await d1.prepare(
    "SELECT * FROM catalog_entities WHERE type = 'view' ORDER BY slug"
  ).all();
  ...
  try {
    ...
    if (action === "update" || action === "create") {
      ...
      if (entity.related && Array.isArray(entity.related)) {
        await d1.prepare("DELETE FROM catalog_relations WHERE from_type = 'view' AND from_slug = ?")
          .bind(row.slug).run();
    ...
    if (action === "delete") {
      ...
      await d1.prepare("DELETE FROM catalog_entities WHERE type = 'view' AND slug = ?")
        .bind(slug).run();
      await d1.prepare("DELETE FROM catalog_relations WHERE from_type = 'view' AND from_slug = ?")
        .bind(slug).run();
      await d1.prepare("DELETE FROM catalog_relations WHERE to_type = 'view' AND to_slug = ?")
        .bind(slug).run();
