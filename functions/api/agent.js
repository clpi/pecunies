...
export async function onRequestGet({ request, env }) {
  ...
  if (slug && slug !== "agent") {
    const result = await d1.prepare(
      "SELECT * FROM catalog_entities WHERE type = 'agent' AND slug = ?"
    ).bind(slug).first();
    ...
    const usedByResult = await d1.prepare(`
      ...
      WHERE cr.to_type = 'agent' AND cr.to_slug = ?
  ...
  const result = await d1.prepare(
    "SELECT * FROM catalog_entities WHERE type = 'agent' ORDER BY slug"
  ).all();
...
export async function onRequestPost({ request, env }) {
  ...
  try {
    ...
    if (action === "delete") {
      ...
      await d1.prepare("DELETE FROM catalog_entities WHERE type = 'agent' AND slug = ?")
        .bind(slug).run();
      await d1.prepare("DELETE FROM catalog_relations WHERE from_type = 'agent' AND from_slug = ?")
        .bind(slug).run();
      await d1.prepare("DELETE FROM catalog_relations WHERE to_type = 'agent' AND to_slug = ?")
        .bind(slug).run();
