...
export async function onRequestGet({ request, env }) {
  ...
  if (slug && slug !== "systemprompt") {
    const result = await d1.prepare(
      "SELECT * FROM catalog_entities WHERE type = 'systemprompt' AND slug = ?"
    ).bind(slug).first();
    ...
    const usedByResult = await d1.prepare(`
      ...
      WHERE cr.to_type = 'systemprompt' AND cr.to_slug = ?
  ...
  const result = await d1.prepare(
    "SELECT * FROM catalog_entities WHERE type = 'systemprompt' ORDER BY slug"
  ).all();
  ...
  try {
    ...
    if (action === "update" || action === "create") {
      ...
      if (entity.related && Array.isArray(entity.related)) {
        await d1.prepare("DELETE FROM catalog_relations WHERE from_type = 'systemprompt' AND from_slug = ?")
          .bind(row.slug).run();
        
        for (const rel of entity.related) {
          await d1.prepare(`
            INSERT INTO catalog_relations (from_type, from_slug, relation_type, to_type, to_slug, label)
            VALUES ('systemprompt', ?, 'related', ?, ?, ?)
    ...
    if (action === "delete") {
      ...
      await d1.prepare("DELETE FROM catalog_entities WHERE type = 'systemprompt' AND slug = ?")
        .bind(slug).run();
      await d1.prepare("DELETE FROM catalog_relations WHERE from_type = 'systemprompt' AND from_slug = ?")
        .bind(slug).run();
      await d1.prepare("DELETE FROM catalog_relations WHERE to_type = 'systemprompt' AND to_slug = ?")
        .bind(slug).run();
