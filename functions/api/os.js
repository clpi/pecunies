...
const PROJECTS = {
  "marketplace-aggregator": {
    ...
    body: "Marketplace Aggregator on AWS is a serverless, message-oriented marketplace aggregation platform at moe.pecunies.com. It uses Lambda, Step Functions, DynamoDB, SQS, API Gateway, CloudFront, and AWS CDK to handle eventual consistency, rate limiting, external marketplace failures, retries, idempotency, and secure webhook ingestion.",
  },
  "webassembly-runtime": {
    aliases: ["zig", "wart", "wasm", "webassembly-runtime"],
    title: "WebAssembly Runtime in Zig",
    body: "The WebAssembly Runtime in Zig is a performance-targeted runtime focused on low-level memory layout and instruction dispatch. It targets nearly full WebAssembly 3.0 and WASI 1 preview specification support and is available at github.com/clpi/wart.git.",
  },
  "pi-cluster": {
    aliases: ["pi", "raspberry-pi", "cluster", "pi-cluster"],
    title: "Raspberry Pi Infrastructure Cluster",
    body: "The Raspberry Pi Infrastructure Cluster is a hardware-constrained home lab for orchestration, observability, and deployment experiments. It is used to test Linux services, Kubernetes, GitOps, monitoring, and infrastructure patterns outside managed cloud defaults.",
  },
  "down-nvim": {
    aliases: ["down", "down.nvim", "notes", "markdown"],
    title: "down.nvim",
    body: "down.nvim is a Neovim 0.12+ plugin for markdown note-taking with planned LSP and AI integration. It focuses on terminal-native capture, navigation, backlinks, structured markdown operations, and future AI-assisted workflows.",
  },
};
...
const DIRECTORIES = {
  ...
  "/home": ["guest/"],
  "/home/guest": ["README.md", ".clpshrc", ".clpsh_history"],
  "/posts": ["2026/"],
  "/posts/2026": ["04/"],
  "/posts/2026/04": ["29/"],
  "/posts/2026/04/29": ["terminal-portfolio-changelog.md"],
  ...
  "/projects": [
    "marketplace-aggregator.md",
    "webassembly-runtime.md",
    "down-nvim.md",
    "pi-cluster.md",
  ],
  "/system": ["man.txt"],
...
const TAGS = {
  ...
  project: [
    {
      ...
      type: "view",
      command: "explain project market",
    },
    {
      label: "WebAssembly Runtime",
      type: "view",
      command: "explain project wasm",
    },
    { label: "down.nvim", type: "view", command: "explain project down" },
    {
      label: "Raspberry Pi Cluster",
      type: "view",
      command: "explain project pi",
    },
  ...
  cloud: [
    ...
    {
      label: "Marketplace Aggregator on AWS",
      type: "view",
      command: "explain project market",
    },
  ],
  ...
  career: [
    ...
    {
      label: "explain education",
      type: "command",
      command: "explain education",
    },
    { label: "whoami", type: "command", command: "whoami" },
  ],
  tooling: [
...
const MANUALS = {
  ...
  sync: "sync\nSync /posts markdown into D1 + R2 snapshots and sync /public/posts files into R2 assets. Requires sudo by default; use POST_CREATION_MODE=open to allow local non-admin sync.",
...
export async function onRequestPost({ request, env }) {
  ...
  const pendingAuth = await handlePendingAuth(
    command,
    state,
    env,
    visibleContext,
  ...
  if (body?.recordOnly) {
    logSystemEvent(
      state,
      `record-only persisted: session=${sessionId} history=${state.history.length}`,
      true,
    );
    await writeState(env, sessionId, state);
    return Response.json(
  ...
  try {
    result = await executeCommandText(
      command,
      state,
      env,
      visibleContext,
...
async function executeCommandText(
  ...
  if (chained.length > 1) {
    ...
    for (const segment of chained) {
      const result = await executeCommandText(
        segment,
        state,
        env,
        visibleContext,
  ...
  if (pipeIndex > 0) {
    ...
    if (leftCommand && rightCommand) {
      const leftResult = await executeCommandText(
        leftCommand,
        state,
        env,
        visibleContext,
  ...
  if (sudoPrefix.command) {
    ...
    const sudoResult = await executeCommandText(
      sudoPrefix.command,
      state,
      env,
      visibleContext,
  ...
  const write = await writeUserFile(
    env,
    state,
    redirected.target,
    result.output ?? "",
...
async function runCommand(
  ...
  if (parsed.name.startsWith("/bin/")) {
    ...
    if (["2048", "chess", "minesweeper", "jobquest"].includes(binCommand)) {
      return {
        output: `Booting ${binCommand}... Type /${binCommand} to start.`,
      };
    }
    if (binCommand === "clpsh") {
  ...
  switch (parsed.name) {
    ...
    case "cat": {
      ...
      const cleanRest =
        parsed.rest.replace("--pretty", "").replace("--pretty", "").trim() ||
        parsed.rest;
      return catPath(cleanRest, state, env, {
        pretty,
        elevated: Boolean(options.elevated),
      });
    }
    case "man": {
      ...
      if (!body)
        return {
          output: "No manual entry. Try man ask or cat /system/man.txt.",
        };
      const tagLine =
      ...
      return {
        output:
          state.history
            .map(
              (entry, index) =>
                `${String(index + 1).padStart(3, " ")}  ${entry.command}`,
            )
            .join("\n") || "(empty)",
      };
    case "ps":
    ...
    case "ask": {
      ...
      const cleanRest = simple
        ? parsed.rest.replace("--simple", "").trim()
        : parsed.rest;
      const question = simple
        ? `Explain this simply and concisely, as if to a junior developer: ${cleanRest}`
        : cleanRest;
    ...
    case "explain": {
      ...
      if (parsed.args[0] === "--simple" && parsed.args[1] === "last") {
        ...
        if (!lastEntry)
          return { output: "explain last: no previous command in history" };
      ...
      if (parsed.args[0] === "last") {
        ...
        return explainWithAiOrFallback(env, state, visibleContext, question, {
          sessionId: sid,
          source: "explain-last",
          ...aiOptions,
        });
...
async function handlePendingAuth(command, state, env, visibleContext, request) {
  ...
  return executeCommandText(
    ...
    state,
    env,
    pending.visibleContext || visibleContext,
    request,
    {
      elevated: true,
    },
...
async function catPath(path, state, env, options = {}) {
  ...
  if (
    (normalized.startsWith("/root/") || normalized.startsWith("/etc/")) &&
    !options.elevated
  ) {
    return {
      output: `cat: ${normalized}: permission denied; use sudo`,
      status: 403,
    };
  }
...
async function explainThing(
  ...
  if (normalizedKind === "command") {
    ...
    if (!manual) {
      return {
        output: `Unknown command "${target}". Try man ${target}.`,
        status: 404,
      };
    }
  ...
  return {
    output: "Usage: explain <project|skill|work|education|command> [name]",
    status: 400,
  };
}
...
async function explainProject(
  ...
  if (!projectName) {
    return {
      output: "Usage: explain project <market|pi|wasm|down>",
      status: 400,
    };
  }

  const normalized = projectName.toLowerCase();
  const project = Object.values(PROJECTS).find((entry) =>
    entry.aliases.includes(normalized),
  );

  if (!project) {
    return {
      output: `Unknown project "${projectName}". Try explain project market, pi, wasm, or down.`,
      status: 404,
    };
  }
...
async function explainWithAiOrFallback(
  env,
  state,
  visibleContext,
  question,
...
function extractAiOptions(rest, state, options = {}) {
  ...
  let system = String(options.systemPrompt || state.config?.system_prompt || "")
    .trim()
    .slice(0, 1200);
...
async function runAi(env, question, state, visibleContext, meta = {}) {
  ...
  const ragKnowledge = await gatherRagKnowledge(
    question,
    state,
    env,
    sessionId,
  ...
  try {
    const result = await env.AI.run(activeModel, {
      messages: [
        {
          role: "system",
          content: `You are a concise terminal AI for Chris Pecunies portfolio. Use only the supplied profile, file, visible, session, metrics, leaderboard, and command-history context. If unknown, say so.${systemInjection ? `\n\nSession system prompt injection:\n${systemInjection}` : ""}`,
        },
        {
          role: "user",
          content: userContent,
    ...
    const out =
      typeof result?.response === "string"
        ? result.response
        : "No text response returned by Workers AI.";
...
function tracerouteHost(host, request) {
  ...
  const lines = [
    `traceroute to ${target.host} (${target.host}), ${hops.length} hops max`,
  ];
  ...
  hops.forEach((hop, index) => {
    ...
    lines.push(
      `${String(index + 1).padStart(2, " ")}  ${String(name).padEnd(24, " ")}  ${p1} ms  ${p2} ms  ${p3} ms`,
    );
  });
...
async function doctorDiagnostics(env, request) {
  ...
  record(
    "Workers AI binding",
    Boolean(env.AI),
    env.AI ? "configured" : "missing",
  );
  record(
    "KV binding (PORTFOLIO_OS)",
    Boolean(env.PORTFOLIO_OS),
    env.PORTFOLIO_OS ? "configured" : "missing",
  );
  record(
    "D1 binding",
    Boolean(env.DB || env.POSTS_DB),
    env.DB || env.POSTS_DB ? "configured" : "missing",
  );
  record(
    "R2 posts bucket",
    Boolean(env.POSTS || env.POSTS_BUCKET),
    env.POSTS || env.POSTS_BUCKET ? "configured" : "missing",
  );
  record(
    "R2 static bucket",
    Boolean(env.STATIC),
    env.STATIC ? "configured" : "missing",
  );
  record(
    "Vectorize binding",
    Boolean(getVectorIndex(env)),
    getVectorIndex(env) ? "configured" : "missing",
  );

  try {
    const res = await fetch("https://pecunies.com", {
      method: "HEAD",
  ...
  try {
    const dns = await fetch(
      "https://dns.google/resolve?name=cloudflare.com&type=A",
      {
        signal: AbortSignal.timeout(4000),
      },
    ).then((res) => res.json());
    ...
    record(
      "DNS resolution",
      ok,
      ok ? "cloudflare.com resolved" : "no A answers",
    );
  } catch {
...
async function weather(location) {
  ...
  if (!place) {
    return {
      output: `weather: no location found for "${location}"`,
      status: 404,
    };
  }
...
async function stock(rawTicker) {
  ...
  const quote = Object.fromEntries(
    columns.map((column, index) => [column, values?.[index] ?? "N/D"]),
  );
...
async function removeFile(path, state, env, options) {
  ...
  if (normalized === "/" || normalized === "") {
    return {
      output: "rm: refusing to operate on / — this is a simulated filesystem",
      status: 403,
    };
  }
  ...
  if (isProtectedPath(normalized) && !options.elevated) {
    return {
      output: `rm: ${normalized}: permission denied; use sudo`,
      status: 403,
    };
  }
...
async function writeUserFile(env, _state, rawPath, content, options = {}) {
  ...
  if (isProtectedPath(path) && !options.elevated) {
    return {
      output: `write: ${path}: permission denied; use sudo`,
      status: 403,
    };
  }
...
async function readFile(path, env, options = {}) {
  ...
  if (path === "/var/log/system.log") {
    if (!options.elevated)
      return "permission denied; use sudo cat /var/log/system.log";
    return Array.isArray(options.logEntries)
      ? options.logEntries.join("\n") || "(no log entries)"
      : "";
  }
  if (path === "/var/log/ai.log") {
    if (!options.elevated)
      return "permission denied; use sudo cat /var/log/ai.log";
    return readAiLogText(env);
  }
  if (path === "/var/log/system_public.log") {
    return Array.isArray(options.publicLogEntries)
...
function canonicalPostPathForStorage(path) {
  ...
  if (
    ...
    normalized.startsWith("/public/posts/")
  ) {
...
function postMirrorPath(path) {
  ...
  if (normalized.startsWith("/public/posts/")) {
    return assetPathToPostPath(normalized);
  }
  return null;
}
...
function emailDraft(args, rest) {
  ...
  const message =
    messageParts.join(" ") || rest.replace(`${from} ${subject}`, "").trim();
...
async function bookMeeting(args, rest, env) {
  const [email = "", date = "", time = "", duration = "", ...messageParts] =
    args;
  const message =
    messageParts.join(" ") ||
    rest.replace(`${email} ${date} ${time} ${duration}`, "").trim();

  if (!email || !date || !time || !duration || !message) {
    return {
      output: "Usage: book <your email> <date> <time> <duration> <message>",
      status: 400,
    };
  }
  ...
  const emailStatus = await sendBookingEmail(env, {
    email,
    date,
    time,
    duration,
...
function uptimeOutput(state) {
  const now = new Date();
  const firstEvent = state.history[0]?.at
    ? Date.parse(state.history[0].at)
    : NaN;
...
function lastOutput(state, rawLimit) {
  const limit = Math.min(
    Math.max(Number.parseInt(String(rawLimit ?? ""), 10) || 10, 1),
    50,
  );
  ...
  for (const game of games) {
    ...
    lines.push(
      ...(scores.length
        ? scores.map(
            (entry, index) =>
              `${index + 1}. ${entry.name} ${entry.score} (${entry.at})`,
          )
        : ["(empty)"]),
    );
    lines.push("");
...
function internet(site) {
  ...
  if (!page) {
    return {
      output: `internet: site not found. Try ${Object.keys(pages).join(", ")}`,
      status: 404,
    };
  }
...
function fzf(query) {
  const haystack = [
    ...Object.keys(MANUALS).map((name) => ({
      label: name,
      detail: MANUALS[name].split("\n")[0],
    })),
  ...
  const scored = haystack
    .map((entry) => ({
      ...entry,
      score: fuzzyScore(entry.label.toLowerCase(), needle),
    }))
  ...
  return {
    output:
      scored.map((entry) => `${entry.label} - ${entry.detail}`).join("\n") ||
      "(empty)",
  };
}
...
async function mergeTagRegistryWithPosts(env) {
  ...
  if (db) {
    try {
      const rows = await db
        .prepare(
          "SELECT t.slug, ti.label, ti.type, ti.command FROM tags t JOIN tag_items ti ON t.slug = ti.tag_slug",
        )
        .all();
...
async function resolvePostIdentifierToPath(identifier, env) {
  ...
  const hits = all.filter(
    (p) =>
      ...
      (p === `/posts/${slug}.md` ||
        p.endsWith(`/${slug}.md`) ||
        p.split("/").pop()?.replace(/\.md$/i, "") === slug.split("/").pop()),
  );
  if (hits.length === 1) {
    return hits[0];
  }
...
async function createNewPost(parsed, env, options) {
  if ((parsed.args[0] || "").toLowerCase() !== "post") {
    return {
      output:
        "Usage: new post --title=<title> --tags=<a,b> [--description=<text>] <body markdown>",
      status: 400,
    };
  }
  const postCreationMode = String(env.POST_CREATION_MODE || "sudo")
    .trim()
    .toLowerCase();
  ...
  if (!tags.length) {
    return {
      output: "new post: --tags= is required (comma-separated list)",
      status: 400,
    };
  }
...
async function uploadPostImage(args, env, options = {}) {
  ...
  if (kind.toLowerCase() !== "image" || !postRef || !imageUrl) {
    return {
      output:
        "Usage: upload image <post-slug|/posts/path.md> <https://image-url> [alt text]",
      status: 400,
    };
  }
  ...
  if (!response.ok) {
    return {
      output: `upload: failed to fetch image (${response.status})`,
      status: 400,
    };
  }
  ...
  if (!contentType.startsWith("image/")) {
    return {
      output: `upload: remote content is not an image (${contentType || "unknown"})`,
      status: 400,
    };
  }
      ...
      : contentType.includes("webp")
        ...
        : contentType.includes("gif")
          ...
          : contentType.includes("svg")
            ...
            : "bin";
  const postSlug =
    resolvedPostPath.split("/").pop()?.replace(/\.md$/i, "") || "post";
  const sourceName = source.pathname.split("/").pop() || "";
  const sourceStem =
    sourceName.replace(/\.[a-z0-9]+$/i, "") || `image-${Date.now()}`;
  const safeStem =
    sourceStem
      .toLowerCase()
      .replace(/[^a-z0-9._-]+/g, "-")
      ...
      .slice(0, 64) || "image";
  const ext =
    (sourceName.split(".").pop() || extGuess)
      .toLowerCase()
      .replace(/[^a-z0-9]/g, "") || extGuess;
  const directory = resolvedPostPath.slice(
    0,
    resolvedPostPath.lastIndexOf("/"),
  );
  const assetPath = postPathToAssetPath(
    `${directory}/${postSlug}-${safeStem}.${ext}`,
  );
  ...
  if (typeof currentMarkdown !== "string") {
    return {
      output: `upload: could not read post ${resolvedPostPath} for markdown update`,
      status: 500,
    };
  }
  ...
  const write = await writeUserFile(env, {}, resolvedPostPath, nextMarkdown, {
    elevated: true,
  });
  if (write.status && write.status >= 400) {
    return write;
...
async function syncPostsAndAssets(env, options = {}) {
  ...
  if (!hasDb && !hasBucket) {
    return {
      output:
        "sync: no storage bindings configured (POSTS_DB|DB and POSTS_BUCKET|POSTS missing).",
      status: 500,
    };
  }
  ...
  for (const [canonicalPath, sourcePath] of postTargets.entries()) {
    try {
      ...
      failures.push(
        `post ${canonicalPath}: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  ...
  for (const assetPath of assetTargets) {
    try {
      ...
      failures.push(
        `asset ${assetPath}: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
...
async function tagsOutput(tagFilter, env) {
  ...
  if (!tagFilter) {
    const lines = keys
      .sort()
      .map((tag) => `${tag.padEnd(16)} (${REG[tag].length} items)`);
    return { output: lines.join("\n") || "(no tags)" };
  }
  ...
  const header =
    matchingKeys.length > 1
      ? `Tags matched: ${matchingKeys.join(", ")}\n\n`
      : "";
  const body = merged
    .map((e) => `  ${e.type.padEnd(8)} ${e.label}  →  ${e.command}`)
    .join("\n");
...
async function copyFile(src, dest, state, env, options = {}) {
  ...
  if (!inAllowed && !options.elevated)
    return {
      output: `cp: ${destPath}: permission denied; use sudo`,
      status: 403,
    };
  if (FILES[destPath] || DIRECTORIES[destPath]) {
    if (!options.force)
      return {
        output: `cp: ${destPath}: file exists (use -f to force)`,
        status: 400,
      };
  }
  ...
  if (
    ...
    !DIRECTORIES[parentDir].some(
      (e) => e.replace(/\/$/, "") === destPath.split("/").pop(),
    )
  ) {
...
async function moveFile(src, dest, state, env, options = {}) {
  ...
  const isProtected = protectedPaths.some(
    (p) => srcPath.startsWith(p) || srcPath === p.replace(/\/$/, ""),
  );
  if (isProtected && !options.elevated)
    return {
      output: `mv: ${srcPath}: permission denied; use sudo`,
      status: 403,
    };
  const newAllowed = ["/home/", "/guest/", "/tmp/"];
  const destAllowed = newAllowed.some((p) => destPath.startsWith(p));
  if (!destAllowed && !options.elevated)
    return {
      output: `mv: ${destPath}: permission denied; use sudo`,
      status: 403,
    };
  FILES[destPath] = content;
  ...
  if (DIRECTORIES[srcDir]) {
    DIRECTORIES[srcDir] = DIRECTORIES[srcDir].filter(
      (e) => e.replace(/\/$/, "") !== srcPath.split("/").pop(),
    );
  }
...
async function linkFile(src, dest, state, env, options = {}) {
  ...
  if (!inAllowed && !options.elevated)
    return {
      output: `ln: ${destPath}: permission denied; use sudo`,
      status: 403,
    };
  FILES[destPath] = symlink
    ? `→ ${srcPath}`
    : FILES[srcPath] || `(dir: ${srcPath})`;
  const destDir = destPath.substring(0, destPath.lastIndexOf("/")) || "/";
  if (
    DIRECTORIES[destDir] &&
    !DIRECTORIES[destDir].includes(destPath.split("/").pop())
  ) {
...
function expandEnvVars(text, state) {
  ...
  return text.replace(/\$\{?(\w+)\}?/g, (match, name) => {
    if (name === "?")
      return String(
        typeof process !== "undefined" ? (process.exitCode ?? 0) : 0,
      );
    if (name === "PWD") return state.cwd || "/";
    if (name === "HOME") return userHomePath(state);
    if (name === "USER")
      return sanitizeUsername(mergeConfigDefaults(state.config).name);
    return envVars[name] ?? "";
  });
}
...
function envValue(name, state) {
  const envVars = state.envVars || {};
  if (name === "?")
    return String(typeof process !== "undefined" ? (process.exitCode ?? 0) : 0);
  if (name === "PWD") return state.cwd || "/";
  if (name === "HOME") return userHomePath(state);
  if (name === "USER")
    return sanitizeUsername(mergeConfigDefaults(state.config).name);
...
function logsOutput(args, state, options) {
  if (args.includes("--full")) {
    if (!options.elevated)
      return {
        output: "logs --full: permission denied; use sudo",
        status: 403,
      };
    return {
      output:
        (Array.isArray(state.logEntries) ? state.logEntries.join("\n") : "") ||
        "(no log entries)",
    };
  }
  return {
    output:
      (Array.isArray(state.publicLogEntries)
        ? state.publicLogEntries.join("\n")
        : "") || "(no public log entries)",
  };
}
...
async function envHandler(args, state, env, options = {}) {
  ...
  if (action === "get") {
    ...
    if (!(key in envVars))
      return { output: `env: ${key} is not set`, status: 404 };
  ...
  if (action === "set") {
    ...
    if (!/^[A-Za-z_][A-Za-z0-9_]*$/.test(key)) {
      return {
        output: "env set: invalid key. Use shell-style names like MY_VAR.",
        status: 400,
      };
    }
  ...
  return {
    output: "Usage: env [list|view|get <KEY>|set <KEY=VALUE>]",
    status: 400,
  };
}
...
async function collectEffectiveEnvVars(state, env, options = {}) {
  ...
  Object.assign(
    result,
    state.envVars && typeof state.envVars === "object" ? state.envVars : {},
  );
  return result;
}
...
function mkdirPath(args, options, state) {
  ...
  if (!allowed && !options.elevated)
    return {
      output: `mkdir: ${normalized}: permission denied; allowed in /home, /guest, /tmp`,
      status: 403,
    };
  if (DIRECTORIES[normalized])
    return { output: `mkdir: ${normalized}: directory exists` };
...
async function persistClpshHistoryFile(env, state) {
  ...
  await env.PORTFOLIO_OS.put(
    userFileKey(`${home}/.clpsh_history`),
    lines.join("\n"),
  );
}
...
function normalizeState(state) {
  ...
  return {
    ...
    pendingAuth:
      s.pendingAuth && typeof s.pendingAuth === "object" ? s.pendingAuth : null,
    rootUntil: Number(s.rootUntil ?? 0),
    logEntries: Array.isArray(s.logEntries) ? s.logEntries : [],
    publicLogEntries: Array.isArray(s.publicLogEntries)
      ? s.publicLogEntries
      : [],
...
async function readState(env, sessionId) {
  ...
  if (db) {
    ...
    const row = await db
      .prepare(
        "SELECT state_json FROM session_state WHERE session_id = ? LIMIT 1",
      )
      .bind(sessionId)
      .first();
...
async function readMetrics(env) {
  ...
  const metrics =
    (await env.PORTFOLIO_OS.get("metrics:global", { type: "json" })) ??
    defaultMetrics();
  return {
    visits: Number(metrics.visits ?? 0),
    pages:
      metrics.pages && typeof metrics.pages === "object" ? metrics.pages : {},
    commands:
      metrics.commands && typeof metrics.commands === "object"
        ? metrics.commands
        : {},
    countries:
      metrics.countries && typeof metrics.countries === "object"
        ? metrics.countries
        : {},
  };
}
...
async function readMetricEvents(env) {
  ...
  do {
    ...
    for (const key of page.keys ?? []) {
      ...
      if (event.type === "page") {
        aggregate.visits += 1;
        aggregate.pages[event.route] =
          Number(aggregate.pages[event.route] ?? 0) + 1;
      }

      if (event.type === "command") {
        aggregate.commands[event.command] =
          Number(aggregate.commands[event.command] ?? 0) + 1;
      }

      const country = event.country ?? "XX";
      aggregate.countries[country] =
        Number(aggregate.countries[country] ?? 0) + 1;
    }
  } while (cursor);
...
async function readLeaderboard(env) {
  ...
  const board =
    (await env.PORTFOLIO_OS.get("leaderboard:global", { type: "json" })) ??
    defaultLeaderboard();
  return {
...
async function recordBooking(env, booking) {
  ...
  const id = `${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
  await env.PORTFOLIO_OS.put(`booking:${id}`, JSON.stringify(booking), {
...
async function writeState(env, sessionId, state) {
  ...
  await env.PORTFOLIO_OS.put(`session:${sessionId}`, JSON.stringify(state), {
...
async function ensureStateInfra(env) {
  ...
  await db
    .prepare(
      "CREATE INDEX IF NOT EXISTS idx_rag_memory_session ON rag_memory(session_id, created_at DESC)",
    )
    .run();
}
...
async function sessionOutput(state, env, request) {
  const metrics = await readMetrics(env);
  const cf = request?.cf ?? {};
  const topCmd = Object.entries(metrics.commands).sort(
...
function noteHandler(args, state) {
  ...
  if (sub === "list") {
    if (!state.notes.length)
      return { output: "No notes yet. Use note add <text> to create one." };
    return {
      output: state.notes
        .map((n, i) => `${i + 1}. [${n.at.slice(0, 10)}] ${n.text}`)
        .join("\n"),
    };
  }
  if (sub === "clear") {
...
async function ragHandler(args, state, env, sessionId = "anonymous") {
  ...
  if (sub === "add" || sub === "remember") {
    ...
    await upsertVectorMemory(env, clipped, {
      sessionId,
      source: "rag-add",
      at,
    });
  ...
  if (sub === "list" || !sub) {
    if (!state.ragContext.length)
      return { output: "rag: no session context notes stored" };
    return {
      output: state.ragContext
        .map(
          (entry, index) =>
            `${index + 1}. [${entry.at.slice(0, 10)}] ${entry.text}`,
        )
        .join("\n"),
    };
  }
  ...
  return {
    output: "Usage: rag <add|list|search|clear> [context]",
    status: 400,
  };
}
...
async function gatherRagKnowledge(
  question,
  state,
  env,
  sessionId = "anonymous",
  ...
  const metricsSnapshot = `visits=${Number(metrics?.visits || 0)} topPages=${Object.entries(
    metrics?.pages || {},
  )
    .slice(0, 5)
    .map(([k, v]) => `${k}:${v}`)
  ...
  const repositoryHits = await queryKnowledge(env, question, { limit: 6 });
  for (const hit of repositoryHits) {
    candidates.push({
      source: `repository:${hit.source}`,
  ...
  return scored
    .map(
      (item, idx) =>
        `${idx + 1}. [${item.source}] ${String(item.text || "").slice(0, 900)}`,
    )
    .join("\n");
}
...
async function persistRagMemory(env, sessionId, text, meta = {}) {
  ...
  await env.PORTFOLIO_OS.put(key, JSON.stringify(payload), {
...
async function queryVectorMemory(env, query, sessionId) {
  ...
  try {
    ...
    return matches
      ...
      .filter(
        (meta) => meta && (!meta.sessionId || meta.sessionId === sessionId),
      )
...
async function updateGuestShellRc(env, config) {
  ...
  await env.PORTFOLIO_OS.put(
    userFileKey(`${home}/.clpshrc`),
    renderGuestShellRc(c),
  );
}
...
async function configHandler(args, state, env) {
  ...
  if (sub === "list" || sub === "get") {
    const prop = args[1];
    if (prop)
      return { output: `${prop}: ${state.config[prop] ?? "(not set)"}` };
    return {
      output: Object.entries(state.config)
  ...
  if (sub === "set" && args[1]) {
    ...
    const prevNameSan = sanitizeUsername(
      String(mergeConfigDefaults(state.config).name),
    );
    if (key === "crt") {
      ...
      if (key === "name") {
        ...
        const normalized = String(val || "")
          .trim()
          .toLowerCase();
        val = ["default", "contrast", "pastel"].includes(normalized)
          ? normalized
          : "default";
  ...
  return {
    output: "Usage: config <set|get|list|reset> [key] [value]",
    status: 400,
  };
}
...
function aliasHandler(args, state) {
  ...
  if (!value)
    return { output: `alias ${name}='${state.aliases[name] ?? "(not set)"}'` };
...
function unaliasHandler(args, state) {
  if (!state.aliases) state.aliases = {};
  if (args.length === 0)
    return { output: "Usage: unalias <name>", status: 400 };
  const name = args[0];
  if (!(name in state.aliases))
    return { output: `unalias: ${name}: not found`, status: 404 };
...
function setVarHandler(args, state) {
  ...
  if (args.length < 2)
    return { output: "Usage: set <key> <value>", status: 400 };
  const key = args[0];
  const value = args.slice(1).join(" ");
