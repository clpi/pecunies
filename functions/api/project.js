...
export async function onRequestGet({ request, env }) {
  ...
  if (slug && slug !== "project") {
    const result = await d1.prepare(
      "SELECT * FROM catalog_entities WHERE type = 'project' AND slug = ?"
    ).bind(slug).first();
    ...
    const usedByResult = await d1.prepare(`
      ...
      WHERE cr.to_type = 'project' AND cr.to_slug = ?
  ...
  const result = await d1.prepare(
    "SELECT * FROM catalog_entities WHERE type = 'project' ORDER BY slug"
  ).all();
...
export async function onRequestPost({ request, env }) {
  ...
  try {
    ...
    if (action === "delete") {
      ...
      await d1.prepare("DELETE FROM catalog_entities WHERE type = 'project' AND slug = ?")
        .bind(slug).run();
      await d1.prepare("DELETE FROM catalog_relations WHERE from_type = 'project' AND from_slug = ?")
        .bind(slug).run();
      await d1.prepare("DELETE FROM catalog_relations WHERE to_type = 'project' AND to_slug = ?")
        .bind(slug).run();
