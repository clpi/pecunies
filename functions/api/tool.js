...
export async function onRequestGet({ request, env }) {
  ...
  if (slug && slug !== "tool") {
    const result = await d1.prepare(
      "SELECT * FROM catalog_entities WHERE type = 'tool' AND slug = ?"
    ).bind(slug).first();
    ...
    const usedByResult = await d1.prepare(`
      ...
      WHERE cr.to_type = 'tool' AND cr.to_slug = ?
  ...
  const result = await d1.prepare(
    "SELECT * FROM catalog_entities WHERE type = 'tool' ORDER BY slug"
  ).all();
  ...
  try {
    ...
    if (action === "update" || action === "create") {
      ...
      if (entity.related && Array.isArray(entity.related)) {
        await d1.prepare("DELETE FROM catalog_relations WHERE from_type = 'tool' AND from_slug = ?")
          .bind(row.slug).run();
    ...
    if (action === "delete") {
      ...
      await d1.prepare("DELETE FROM catalog_entities WHERE type = 'tool' AND slug = ?")
        .bind(slug).run();
      await d1.prepare("DELETE FROM catalog_relations WHERE from_type = 'tool' AND from_slug = ?")
        .bind(slug).run();
      await d1.prepare("DELETE FROM catalog_relations WHERE to_type = 'tool' AND to_slug = ?")
        .bind(slug).run();
